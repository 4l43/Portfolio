...
int hide(){
	...
	printf("Entrer la phrase à cahcer \n max %d cahracteres : \n ",tailleMAX*4);
	scanf("%s",phrase);
	
	
	//transformons la chaine en binaire ( plustot son equivalent decimale) 
	int i= 0,j=0,temp;
	do {
	for (int z=7;z>=0;z--){
            temp=phrase[j]>>z&1;
...
int decode(){
   //demandons à l'utilisateur le fichier à modifier 
   char nom[40];
   printf("Entrer le nom du fichier a decoder : \n");
   scanf("%s",nom);
   ...
   int i;
   int *tab1 = malloc(sizeof(int)*h.taille);
   int *octet = malloc(sizeof(int)*h.taille);
   ...
   //applicons la stegano:
   int j=0,L=0;
   //ne fonctioonne pas sous windows
   for(i = h.offset-1; i < h.taille; i=i+h.padding){
         for(L = 1; L<=h.longueur; L++){
            i=i+3;
			tab1[i] = ((tab1[i]%4)*4);
...
int main(){
	char verif;
	printf("souhaitez-vous steganogrephier : S ou decoder une image : L ? : \n");
	scanf("%c",&verif);
	if (verif == "S"){
		hide();
	}
